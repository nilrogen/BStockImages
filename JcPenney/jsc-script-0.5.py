...
LINEREGEX = re.compile('Item List for Pallet (.*)')
...
def typeLine(splitline):
    if len(splitline) == 5 and splitline[-1] != 'Weight':
        return ITEMLINE
    elif len(splitline) == 1 and splitline != ['']:
        if LINEREGEX.match(splitline[0]):
            return PALLETLINE
    return -1
...
def processManifest(manifestname, outdir):
    ...
    with open(manifestname, 'r') as fin:
        ...
        for line in fin:
            ...
            tl = typeLine(spline)
            if tl == PALLETLINE:
                palletnum = LINEREGEX.search(spline[0]).group(1)
                if curmanifest.palletnum != palletnum:
                    manifests.append(curmanifest)
                    curmanifest = Manifest(palletnum, city)
            elif tl == ITEMLINE:
